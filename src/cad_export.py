...
# NOTE: interior_rules.py (furniture placement) isn't included in this copy
# of the project, so `furniture` items are duck-typed here (any object with
# .x, .y, .w, .h, .name) instead of importing FurnitureItem from it.
FurnitureItem = Any
...
def demo():
    """Loads layout.json (written by csp_generator.demo()) and exports it
    to a DXF file, with walls extruded to 2.8m."""
    import json
    ...
    assignment = {
        r["name"]: Rect(x=r["x"], y=r["y"], w=r["w"], h=r["h"])
        for r in data["rooms"]
    }

    path = export_floorplan_dxf(assignment, out_path="floorplan.dxf", wall_height=2.8)
